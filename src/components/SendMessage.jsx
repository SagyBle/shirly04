        ...
        <div>
          <div className="suggestion-line-div">
            <div className="suggestion-add-song-div">
              <button  onClick={()=>this.sendMessage(item)} className="suggestion-add-song-button"><img  className="suggestion-add-song-icon" src={Plus} alt="" /></button>
              <div className="suggestion-was-before-div">{this.props.messages.some(message=> message.text === item) && <p className="suggestion-was-before-p">השיר עומד להתנגן בקרוב</p> || this.props.history && this.props.history.some((message, index)=>this.handleHistory(message, index, item)) &&  <p className="suggestion-was-before-p">נוגן לפני כמה שירים</p>}  </div>
            </div>
  ...
  render() {
    ...
    return (
      ...
      <div className="search-song-form">
        {this.props.addRequests ? <input className="search-song-box" placeholder="הקלידו ובחרו מהשירים הקיימים" value={text} onChange={this.onTextChange} type='text' />
        : <input className="search-song-box" placeholder="הוספת שירים כרגע מושהת ע״י מנהלי החדר" value="" /> }
